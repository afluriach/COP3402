...
typedef enum
{
    nulsymbol=0, variable, constant, procedure, array
} SymbolType;
		...
		case identsym:
            ...
            Token t = crntToken();
             ...
             switch(s.type)
             {
                  ...
                  case procedure:
                        error(21, "Expression must not contain procedure identifier."); break;
                  case variable:
...
void statement()
{
	...
	switch(crnt)
	{
		...
		case insym:
            ...
            t   = crntToken();
            Symbol s = getSymbol(t.name);
            if(s.type != variable && s.type != array)
               error(31, "variable expected.");
