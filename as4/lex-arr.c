...
char reservedWords[N_RESERVED_WORDS][MAX_RESERVED_LEN] = {"begin", "end", "if", "then", "while","do", "call", "const", "int", "procedure", "write", "read", "else", "odd"};
...
void lexemeStr(char * dest, Token crnt)
{
		...
		//print symbol including "odd" which is in the symbol range
			...
			(crnt.type == 8) ? strcpy(dest, "odd") : strcpy(dest, symbols[crnt.type - SYMBOL_OFFSET]); 
		//print reserved word
		else
