...
//need to:
//
//add a stack top to the CPU so the full contents of the stack can be printed out beyond sp
//add an array to CPU or change stack to struct; add array to keep track of boundaries
...
void sou (Stack * stack, Cpu * cpu)
{
    if(spCheck(cpu->sp-1)) return;
    printf("%d ", stack->data[cpu->sp--]);
}

void sin1 (Stack * stack, Cpu * cpu)
{
    if(spCheck(cpu->sp+1)) return;
    char buf[12];
...
void cal(Stack * stack, Cpu* cpu)
{
    ...
    stack->frames[stack->frameDepth++] = cpu->sp+1;
...
IsaFunc operations[11] = {NULL, lit, opr, lod, sto, cal, inc, jmp, jpc, sin1, sou};

char opName [][4]  = { "", "LIT", "OPR", "LOD", "STO", "CAL", "INC", "JMP", "JPC", "SIN", "SOU"};
