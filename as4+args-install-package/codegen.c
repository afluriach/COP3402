             ...
             switch(s.type)
             {
				  ...
				  case intProcedure:
					...
					if(s.nArgs >0)
					{
						if(nextToken().type != lparentsym)
							error(26, "26. Left parenthesis expected.");
						emit(INC, 0, AR_SIZE);
						int nArgs = 0;
						while(1){
							nextToken();
						...
						if(nArgs != s.nArgs)
							error(27, "invalid number of arguments");
                  ...
                  case array:
                        ...
                        if(nextToken().type != lbracket)
                            error(28, "Open bracket expected.");
                        nextToken();
                        expression();
                        if(crntToken().type != rbracket)
                            error(29, "Close bracket expected.");
...
int varDeclaration(ArrayL * al, int offset)
{
	...
	if(crntToken().type != identsym)
		error(4, "int must be followed by an identifier.");
	
	do
	{
		Token t = crntToken();
		if(isDeclared(t.name))
			error(30, "identifier has already been declared in this scope.");
		if(peekToken().type == lbracket)
		{
            ...
            if(crntToken().type != numbersym)
                error(31, "array dimension missing.");
...
procDeclaration()
{
    ...
    while(crntToken().type == procsym)
    {
		...
		if(crntToken().type != voidsym && crntToken().type != intsym)
			error(32, "procedure return type missing.");
        ...
        else
        {
            Token t= crntToken();
			if(isDeclared(t.name))
				error(30, "identifer has already been decalred in this scope.");
		...
		if(peekToken().type == lparentsym)
		{
			...
			do{
				...
				Token t = crntToken();
				if(t.type != identsym)
					error(33, "identifier expected in procedure parameter list");
...
void statement()
{
	...
	switch(crnt)
	{
		...
		case identsym:
            ...
            {
			    if(nextToken().type != becomessym)
				    error(35, "error, := expected after l-value in statement.");
				    nextToken();
			     expression();
		...
		case callsym:
            ...
            if(s.type != intProcedure && s.type != voidProcedure)
               error(36, "identifier is not callable.");
			nextToken();
			if(s.nArgs >0)
			{
				if(crntToken().type != lparentsym)
					error(26, "left parenthesis missing.");
				emit(INC, 0, AR_SIZE);
				int nArgs = 0;
				do{
					nextToken();
		...
		case insym:
			...
			switch(s.type)
			{
				...
				case procsym:
					...
					error(12, "cannot assign value to constant or procedure.");
					break;
				case variable:
					emit(STO, s.lex, s.value);
					break;
...
int main(int argc, char ** argv)
{
	...
	for(i=0; !feof(fin); i++, mainList->len++)
	{
		...
		fscanf(fin, "%d", &(mainList->list[i].lineNumber));
	}
